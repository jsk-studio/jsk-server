...
const SQL1 = createSampleSQL('table_1', [
    ...
    { name: 'kana', type: 'readonly' },
    { name: 'testName', column: 't_name', type: 'editable' },
])
...
const SAMPLE_SQLS2: string[][] = [[
    ...
    SQL1.SELECT('INC', ['id']).WHERE({ testName: 1 }).END(), 
    ...
    SQL1.SELECT("INC", ['id']).ORDER(['id.desc', 'kana.asc', 'testName.unique']).LIMIT(1).END(), 
